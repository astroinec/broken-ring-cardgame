...
static var DEFINITIONS: Dictionary = {
	...
	&"index_reorder": _entry(&"index_reorder_plus", "查看抽牌堆顶 5 张，将其中 1 张置入弃牌堆，其余顺序不变。消逝。", {&"look_count": 5}),
	&"unsigned_support": _entry(&"unsigned_support_plus", "获得 8 点格挡。若本回合有牌从封存区解封，再获得 5 点格挡。", {&"base_block": 8}),
	&"rift_slash": _entry(&"rift_slash_plus", "造成 15 点伤害；超载 2。", {&"damage": 15}),
	&"forced_stability": _entry(&"forced_stability_plus", "不稳定减少 3；每实际减少 1 点，获得 3 点格挡。", {&"block_per_instability": 3}),
	...
	&"delayed_guard": _entry(&"delayed_guard_plus", "封存 1。解封：获得 16 点格挡。", {&"unseal_block": 16}),
	&"countdown_scar": _entry(&"countdown_scar_plus", "封存 1。解封：对生命最低的敌人造成 18 点伤害。", {&"sealed_turns": 1}),
	&"prewritten_ending": _entry(&"prewritten_ending_plus", "费用 1。选择手牌中 1 张非消逝牌的复制品封存 1；原牌本回合费用变为 0。消逝。", {&"cost": 1}),
	&"unseal_order": _entry(&"unseal_order_plus", "选择 1 张封存牌，其倒计时立即归零并触发解封。", {&"overload": 0}),
